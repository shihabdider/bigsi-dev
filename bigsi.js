...
async function main(){
    ...
    const [bigsi, bigsiDims, ignoreMinimizers] = await makeBigsi.main(fasta)
    ...
    writeBigsi.writeToJSON(bigsiDims, `${argv.output}_query_config.json`)
    writeBigsi.writeToJSON(ignoreMinimizers, `${argv.output}_ignore_minimizers.json`)

    const bucketToPosition = await writeBigsi.makeBucketToPositionMap(fasta)
    writeBigsi.writeToJSON(bucketToPosition, `${argv.output}_bucket_map.json`)
