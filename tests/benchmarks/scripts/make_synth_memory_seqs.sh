...
function make_synth_query() {
    ...
    do
        local output=seqs/synthetic/${dir}/${ref_size}_${query_size}_query_000.fasta
        python3 scripts/make_benchmark_seqs.py \
            -i scripts/synth_acn.txt \
            -l ${query_size} \
            -n ${num_queries_per_seq} \
            -o ${output}\
            -f seqs/synthetic/${dir}/${ref_size}.fasta \
            -x seqs/synthetic/${dir}/${ref_size}.fasta.fai;
...
function mutate_synth_seq() {
    ...
    do
        ...
        do
            ...
            output=seqs/synthetic/${dir}/${ref_size}_${query_size}_query_${sub_rates[i]}.fasta;
            python3 scripts/mutate_seqs.py \
                -i seqs/synthetic/${dir}/${ref_size}_${query_size}_query_000.fasta \
                -r ${sub_rates[i]} \
                -o ${output};
            samtools faidx ${output} \
            &
