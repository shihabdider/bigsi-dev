...
async function getBinaryDumpBigsiSubmatrix(filename, rowFilter, numCols){
    ...
    try {
        ...
        const array = new Uint1Array(bigsiRowBuf.buffer);

        for (let i = 0; i < rowFilter.length; i++){

            const offsetStart = rowFilter[i]*numCols
            const offsetEnd = offsetStart + numCols
            const row = array.slice(offsetStart, offsetEnd).join('')
            const bs = new BitSet(row)
            submatrix.push(bs);
        }
    } catch(err) {
        console.log(err)
